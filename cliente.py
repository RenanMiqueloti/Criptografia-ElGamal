...
def main():
    ...
    server_host = "localhost" # Se o servidor estiver não estiver no mesmo computador troque "localhost" por "ip do servidor" ex: "192.168.0.11"
    server_port = 5555

    client_chave_publica = str(str(chave_publica[0])+"."+str(chave_publica[1])+"."+str(chave_publica[2])).encode("UTF-8")
    sockobj = socket(AF_INET, SOCK_STREAM)
    sockobj.connect((server_host, server_port))

    for linha in [client_chave_publica]:
        sockobj.send(linha)

    server_chave_publica = sockobj.recv(1024)
    server_chave_publica = server_chave_publica.decode().split(".")
    chave_publica = [int(server_chave_publica[0]),int(server_chave_publica[1]),int(server_chave_publica[2])]

    mensagem = criptografar(chave_publica,"ok",tabela_conversao)
